DROP PROCEDURE IF EXISTS reports.`GetPricesByRegionMaskByTypes`;
CREATE DEFINER=`RootDBMS`@`127.0.0.1` PROCEDURE reports.`GetPricesByRegionMaskByTypes`(in inFilter varchar(5000), in inID bigint, in inTypes varchar(255))
BEGIN
declare filterStr varchar(5000);
...
select
	...
	from usersettings.pricesdata pd
  		inner join Customers.suppliers supps on pd.FirmCode = supps.id
	...
	where
		...
		and pd.agencyenabled = 1
 		and prd.enabled = 1
    and pd.PriceType not in (", inTypes, ")
