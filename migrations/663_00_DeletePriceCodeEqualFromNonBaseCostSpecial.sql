...
CREATE temporary table reports.TmpReportCode(
  ...
  (select id from reports.report_type_properties rt where rt.reporttypecode=1 and propertyname='ByBaseCosts') and propertyvalue=0;
  
 delete from reports.report_properties where
PropertyID in (select id from reports.report_type_properties rt where rt.reporttypecode=1 and rt.propertyname='PriceCodeEqual') 
