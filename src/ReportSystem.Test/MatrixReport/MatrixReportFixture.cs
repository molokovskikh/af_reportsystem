...
value
({0}, {1}, {2})
", price.Id, product.Id, matrix.Id)).ExecuteUpdate();
				clientId = client.Id;
			});
			...
			var result = File.ReadAllText(string.Format("Rep.xls"));
			Assert.That(((MatrixReportForTest)report).Result.Tables["Results"].Rows[4]["F13"], Is.StringContaining("Удаление предложения"));
			Assert.That(result, Is.StringContaining(producerName));
		}
	}
}
